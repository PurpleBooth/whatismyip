...
fn run_with_args(args: &[&str]) -> Result<String> {
    ...
    let output = Command::new("cargo")
        ...
        .args(args)
        .output()
        .map_err(|e| miette!("Failed to execute cargo run with args {args:?}: {e}"))?;

    // Check that the program executed successfully
    if !output.status.success() {
        return Err(miette!("Program execution failed with args: {args:?}"));
    }
    ...
    if stdout.trim().is_empty() {
        return Err(miette!("Program output is empty with args: {args:?}"));
    }
